...
local soundReplacements = {
	...
	[114805617662705] = getcustomasset("theforsaken/Assets/base.frsk3") --base
}

local chaseReplacements = {
	[14623473379] = getcustomasset("theforsaken/Assets/sonichase.frsk3") -- sonic.exe chase
}

local sfxReplacements = {
	[134524676238762] = getcustomasset("theforsaken/Assets/kollocharge.frsk3") -- kollosos charge
}
...
-- literally the same but for chases

local function replaceChaseId()
    local currentSound = SoundService.mainSounds:FindFirstChild("chase")
    if currentSound and currentSound:IsA("Sound") then
        local currentId = tonumber(currentSound.SoundId:match("%d+")) -- Extract numeric part
        if chaseReplacements[currentId] then
            currentSound.SoundId = chaseReplacements[currentId] -- Replace with new ID
            print("Replaced ChaseId:", currentId, "->", currentSound.SoundId)
        end
    end
end

-- literally the same but for exe sfx



-- Infinite loop to constantly check and replace sound
RunService.Heartbeat:Connect(replaceSoundId)
RunService.Heartbeat:Connect(replaceChaseId)
